...
-- CreateTable
CREATE TABLE [dbo].[LershaFarmer] (
    [id] NVARCHAR(1000) NOT NULL,
    [farmerId] NVARCHAR(1000) NOT NULL,
    [farmerName] NVARCHAR(1000) NOT NULL,
    [phoneNumber] NVARCHAR(1000) NOT NULL,
    [kebeleIdDocUrl] NVARCHAR(max) NOT NULL,
    [landCertificateDocUrl] NVARCHAR(max) NOT NULL,
    [totalFarmSizeInHectare] FLOAT(53) NOT NULL,
    [cultivatedAreaInHectare] FLOAT(53) NOT NULL,
    [primaryCropType] NVARCHAR(1000) NOT NULL,
    [farmRegistryNumber] NVARCHAR(1000) NOT NULL,
    [requestedLoanAmount] FLOAT(53) NOT NULL,
    [repaymentSource] NVARCHAR(1000) NOT NULL,
    [requestedLoanTermInMonth] INT NOT NULL,
    [applicationChannel] NVARCHAR(1000) NOT NULL,
    [creditScoreValue] FLOAT(53) NOT NULL,
    [scoreCalculationDate] DATETIME2 NOT NULL,
    [emergencyContactName] NVARCHAR(1000) NOT NULL,
    [emergencyContactPhone] NVARCHAR(1000) NOT NULL,
    [emergencyContactRelationship] NVARCHAR(1000) NOT NULL,
    [emergencyContactAddress] NVARCHAR(max) NOT NULL,
    [status] NVARCHAR(1000) NOT NULL CONSTRAINT [LershaFarmer_status_df] DEFAULT 'PENDING',
    [marriageCertificateUrl] NVARCHAR(max),
    [address] NVARCHAR(max) NOT NULL,
    [createdAt] DATETIME2 NOT NULL CONSTRAINT [LershaFarmer_createdAt_df] DEFAULT CURRENT_TIMESTAMP,
    [updatedAt] DATETIME2 NOT NULL,
    CONSTRAINT [LershaFarmer_pkey] PRIMARY KEY CLUSTERED ([id]),
    CONSTRAINT [LershaFarmer_farmerId_key] UNIQUE NONCLUSTERED ([farmerId])
);

-- CreateTable
CREATE TABLE [dbo].[LershaLoanPurpose] (
    [id] NVARCHAR(1000) NOT NULL,
    [farmerId] NVARCHAR(1000) NOT NULL,
    [productId] NVARCHAR(1000),
    [loanPurpose] NVARCHAR(1000) NOT NULL,
    [specificVarietyName] NVARCHAR(1000),
    [quantity] FLOAT(53),
    [unitOfMeasurement] NVARCHAR(1000),
    [unitPrice] FLOAT(53),
    [totalCost] FLOAT(53) NOT NULL,
    [agroDealerName] NVARCHAR(1000),
    [agroDealerAccountNo] NVARCHAR(1000),
    [insuranceName] NVARCHAR(1000),
    [createdAt] DATETIME2 NOT NULL CONSTRAINT [LershaLoanPurpose_createdAt_df] DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT [LershaLoanPurpose_pkey] PRIMARY KEY CLUSTERED ([id]),
    CONSTRAINT [LershaLoanPurpose_productId_key] UNIQUE NONCLUSTERED ([productId])
);

-- CreateTable
CREATE TABLE [dbo].[LershaLoanRequest] (
    [id] NVARCHAR(1000) NOT NULL,
    [farmerId] NVARCHAR(1000) NOT NULL,
    [productId] NVARCHAR(1000) NOT NULL,
    [otp] NVARCHAR(1000),
    [otpExpiresAt] DATETIME2,
    [otpVerified] BIT NOT NULL CONSTRAINT [LershaLoanRequest_otpVerified_df] DEFAULT 0,
    [status] NVARCHAR(1000) NOT NULL CONSTRAINT [LershaLoanRequest_status_df] DEFAULT 'PENDING_OTP',
    [referenceNo] NVARCHAR(1000),
    [lershaDecisionSentAt] DATETIME2,
    [lershaDecisionResponse] NVARCHAR(max),
    [remainingBalance] FLOAT(53),
    [disbursementConfirmedAt] DATETIME2,
    [createdAt] DATETIME2 NOT NULL CONSTRAINT [LershaLoanRequest_createdAt_df] DEFAULT CURRENT_TIMESTAMP,
    [updatedAt] DATETIME2 NOT NULL,
    CONSTRAINT [LershaLoanRequest_pkey] PRIMARY KEY CLUSTERED ([id]),
    CONSTRAINT [LershaLoanRequest_referenceNo_key] UNIQUE NONCLUSTERED ([referenceNo])
);

-- CreateTable
CREATE TABLE [dbo].[PhoneChangeRequest] (
    [id] NVARCHAR(1000) NOT NULL,
    [oldPhoneNumber] NVARCHAR(1000) NOT NULL,
    [newPhoneNumber] NVARCHAR(1000) NOT NULL,
    [reason] NVARCHAR(max) NOT NULL,
    [status] NVARCHAR(1000) NOT NULL CONSTRAINT [PhoneChangeRequest_status_df] DEFAULT 'PENDING',
    [requestedById] NVARCHAR(1000) NOT NULL,
    [approvedById] NVARCHAR(1000),
    [approvedAt] DATETIME2,
    [rejectionReason] TEXT,
    [createdAt] DATETIME2 NOT NULL CONSTRAINT [PhoneChangeRequest_createdAt_df] DEFAULT CURRENT_TIMESTAMP,
    [updatedAt] DATETIME2 NOT NULL,
    CONSTRAINT [PhoneChangeRequest_pkey] PRIMARY KEY CLUSTERED ([id])
);
...
-- CreateIndex
CREATE NONCLUSTERED INDEX [LershaFarmer_phoneNumber_idx] ON [dbo].[LershaFarmer]([phoneNumber]);

-- CreateIndex
CREATE NONCLUSTERED INDEX [LershaFarmer_status_idx] ON [dbo].[LershaFarmer]([status]);

-- CreateIndex
CREATE NONCLUSTERED INDEX [LershaLoanPurpose_farmerId_idx] ON [dbo].[LershaLoanPurpose]([farmerId]);

-- CreateIndex
CREATE NONCLUSTERED INDEX [LershaLoanRequest_farmerId_idx] ON [dbo].[LershaLoanRequest]([farmerId]);

-- CreateIndex
CREATE NONCLUSTERED INDEX [LershaLoanRequest_status_idx] ON [dbo].[LershaLoanRequest]([status]);

-- CreateIndex
CREATE NONCLUSTERED INDEX [LershaLoanRequest_referenceNo_idx] ON [dbo].[LershaLoanRequest]([referenceNo]);

-- CreateIndex
CREATE NONCLUSTERED INDEX [PhoneChangeRequest_status_idx] ON [dbo].[PhoneChangeRequest]([status]);

-- CreateIndex
CREATE NONCLUSTERED INDEX [PhoneChangeRequest_oldPhoneNumber_idx] ON [dbo].[PhoneChangeRequest]([oldPhoneNumber]);

-- CreateIndex
CREATE NONCLUSTERED INDEX [PhoneChangeRequest_newPhoneNumber_idx] ON [dbo].[PhoneChangeRequest]([newPhoneNumber]);
...
-- AddForeignKey
ALTER TABLE [dbo].[LershaLoanPurpose] ADD CONSTRAINT [LershaLoanPurpose_farmerId_fkey] FOREIGN KEY ([farmerId]) REFERENCES [dbo].[LershaFarmer]([id]) ON DELETE CASCADE ON UPDATE CASCADE;

-- AddForeignKey
ALTER TABLE [dbo].[LershaLoanRequest] ADD CONSTRAINT [LershaLoanRequest_farmerId_fkey] FOREIGN KEY ([farmerId]) REFERENCES [dbo].[LershaFarmer]([id]) ON DELETE NO ACTION ON UPDATE NO ACTION;
